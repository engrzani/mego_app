# modules\domainsettingsMJ.psm1
# Domein/DC, OU's, groepen, users, shares, NTFS
# ASCII, PS 5.1

if (-not (Get-Command Write-Log -ErrorAction SilentlyContinue)) {
    throw "Write-Log niet gevonden. Importeer modules\loggingMJ.psm1 voor deze module."
}
...
function Install-DomainController {
    ...
    try { $conf = Get-DomainConfig } catch { Write-Log "FOUT domeinconfig: $_"; return }
    ...
    try {
        ...
        foreach ($f in $need) {
            ...
            if ($feat -and $feat.InstallState -ne 'Installed') {
                ...
                Write-Log "Feature geinstalleerd: $f"
            }
        }
    } catch { Write-Log "FOUT features: $($_.Exception.Message)"; return }
    ...
    # Detecteer domein
    $domainBestaat = $false
    try { $null = Get-ADDomain -Server $conf.Fqdn -ErrorAction Stop; $domainBestaat = $true; Write-Log "Domein bestaat: $($conf.Fqdn)" }
    catch { Write-Log "Geen bestaand domein gevonden: $($conf.Fqdn). Nieuw forest zal worden aangemaakt." }
    ...
    try {
        if ($domainBestaat) {
            ...
            Write-Log "Gepromoveerd tot extra DC in: $($conf.Fqdn)"
        } else {
            ...
            Write-Log "Nieuw forest + eerste DC aangemaakt: $($conf.Fqdn)"
        }

        # Menu automatisch laten starten na reboot
        ...
        Write-Log "RunOnce voor menu gezet (na DC-reboot)."
        shutdown /r /t 5
    } catch {
        Write-Log "FOUT ADDS-promotie: $($_.Exception.Message)"
    }
}

function New-OrganizationalUnits {
    $csvPath = Join-Path $PSScriptRoot '..\settings\ous.csv'
    if (-not (Test-Path $csvPath)) { Write-Log "FOUT: ous.csv niet gevonden: $csvPath"; return }
    try { $conf = Get-DomainConfig } catch { Write-Log $_; return }

    $rows = Import-Csv -Path $csvPath -Delimiter ';'
    foreach ($r in $rows) {
        $ouName    = ("$($r.Name)").Trim()
        $ouPathRaw = ("$($r.Path)").Trim()
        if ([string]::IsNullOrWhiteSpace($ouName)) { Write-Log "Lege OU-naam overgeslagen."; continue }
        ...
        if ($ouPathRaw) {
            ...
            foreach ($seg in $segments) {
                ...
                if (-not (Get-ADOrganizationalUnit -LDAPFilter "(distinguishedName=$segDn)" -ErrorAction SilentlyContinue)) {
                    try { New-ADOrganizationalUnit -Name $seg -Path $parent -ProtectedFromAccidentalDeletion $false; Write-Log "OU aangemaakt: $seg onder $parent" }
                    catch { Write-Log "FOUT OU '$seg' onder ${parent}: $($_.Exception.Message)" }
                }
                $parent = $segDn
            }
        }
        ...
        if (-not (Get-ADOrganizationalUnit -LDAPFilter "(distinguishedName=$ouDn)" -ErrorAction SilentlyContinue)) {
            try { New-ADOrganizationalUnit -Name $ouName -Path $parent -ProtectedFromAccidentalDeletion $false; Write-Log "OU aangemaakt: ${ouDn}" }
            catch { Write-Log "FOUT OU '$ouName' onder ${parent}: $($_.Exception.Message)" }
        } else {
            Write-Log "OU bestaat al: ${ouDn}"
        }
    }
}
...
function New-SecurityGroups {
    ...
    if (-not (Test-Path $csvPath)) { Write-Log "FOUT: securitygroups.csv niet gevonden: $csvPath"; return }
    try { $conf = Get-DomainConfig } catch { Write-Log $_; return }
    ...
    foreach ($g in $groups) {
        ...
        $ou   = ("$($g.ou)").Trim()
        if ([string]::IsNullOrWhiteSpace($name) -or [string]::IsNullOrWhiteSpace($ou)) { Write-Log "Lege groepsnaam/OU overgeslagen."; continue }
        ...
        foreach ($seg in $segments) {
            $segDn = "OU=$seg,$parent"
            if (-not (Get-ADOrganizationalUnit -LDAPFilter "(distinguishedName=$segDn)" -ErrorAction SilentlyContinue)) {
                try { New-ADOrganizationalUnit -Name $seg -Path $parent -ProtectedFromAccidentalDeletion $false; Write-Log "OU aangemaakt voor groep: $seg" }
                catch { Write-Log "FOUT OU '$seg' voor groep '$name': $($_.Exception.Message)" }
            }
            $parent = $segDn
        }
        ...
        if (-not (Get-ADGroup -LDAPFilter "(cn=$name)" -SearchBase $parent -ErrorAction SilentlyContinue)) {
            try { New-ADGroup -Name $name -GroupScope $scope -GroupCategory Security -Path $parent; Write-Log "Groep aangemaakt: $name ($scope) in $parent" }
            catch { Write-Log "FOUT groep '$name': $($_.Exception.Message)" }
        } else { Write-Log "Groep bestaat al: $name in $parent" }
    }
}

function New-DomainUsers {
    $jsonPath = Join-Path $PSScriptRoot '..\settings\users.json'
    if (-not (Test-Path $jsonPath)) { Write-Log "FOUT: users.json niet gevonden: $jsonPath"; return }
    try { $conf = Get-DomainConfig } catch { Write-Log $_; return }
    ...
    foreach ($u in $users) {
        ...
        if (Get-ADUser -LDAPFilter "(sAMAccountName=$sam)" -ErrorAction SilentlyContinue) { Write-Log "Gebruiker bestaat al: $sam"; continue }
        ...
        try {
            ...
            Write-Log "Gebruiker aangemaakt: $sam ($full) in $ouDn"
        } catch { Write-Log "FOUT user '$sam': $($_.Exception.Message)" }
    }
}
...
function Add-UsersToSecurityGroups {
    ...
    foreach ($u in $users) {
        ...
        if (-not $usr) { Write-Log "Gebruiker niet gevonden: $sam"; continue }

        foreach ($grp in $u.securityGroups) {
            $g = Get-ADGroup -LDAPFilter "(cn=$grp)" -ErrorAction SilentlyContinue
            if (-not $g) { Write-Log "Groep niet gevonden: $grp (user $sam)"; continue }
            ...
            if ($already) { Write-Log "$sam is al lid van $grp"; continue }

            try { Add-ADGroupMember -Identity $g.DistinguishedName -Members $usr.DistinguishedName -ErrorAction Stop; Write-Log "$sam -> $grp" }
            catch { Write-Log "FOUT lidmaatschap $sam -> $grp : $($_.Exception.Message)" }
        }
    }
}
...
function New-FolderStructure {
    ...
    if (-not (Test-Path $pathFile)) { Write-Log "FOUT: mappen.txt niet gevonden: $pathFile"; return }

    Get-Content -Path $pathFile | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | ForEach-Object {
        $p = $_.Trim()
        if (-not (Test-Path $p)) {
            try { New-Item -Path $p -ItemType Directory -Force | Out-Null; Write-Log "Map aangemaakt: $p" }
            catch { Write-Log "FOUT map '$p': $($_.Exception.Message)" }
        } else { Write-Log "Map bestaat al: $p" }
    }
}

function New-NetworkShares {
    $csvPath = Join-Path $PSScriptRoot '..\settings\shares.csv'
    if (-not (Test-Path $csvPath)) { Write-Log "FOUT: shares.csv niet gevonden: $csvPath"; return }

    $rows = Import-Csv -Path $csvPath -Delimiter ';'
    foreach ($r in $rows) {
        $folder = ("$($r.map)").Trim()
        $name   = ("$($r.share)").Trim()
        if ([string]::IsNullOrWhiteSpace($folder) -or [string]::IsNullOrWhiteSpace($name)) { Write-Log "Lege map/share overgeslagen"; continue }

        if (-not (Test-Path $folder)) {
            try { New-Item -Path $folder -ItemType Directory -Force | Out-Null; Write-Log "Map aangemaakt voor share: $folder" }
            catch { Write-Log "FOUT map '$folder': $($_.Exception.Message)"; continue }
        }

        if (-not (Get-SmbShare -Name $name -ErrorAction SilentlyContinue)) {
            try { New-SmbShare -Name $name -Path $folder -FullAccess 'BUILTIN\Administrators' | Out-Null; Write-Log "Share aangemaakt: $name -> $folder" }
            catch { Write-Log "FOUT share '$name': $($_.Exception.Message)" }
        } else { Write-Log "Share bestaat al: $name" }
    }
}

function Set-NTFSPermissions {
    $csvPath = Join-Path $PSScriptRoot '..\settings\rechten.csv'
    if (-not (Test-Path $csvPath)) { Write-Log "FOUT: rechten.csv niet gevonden: $csvPath"; return }
    try { $conf = Get-DomainConfig } catch { Write-Log $_; return }
    ...
    foreach ($r in $rows) {
        ...
        if (-not $adg) { Write-Log "Groep ontbreekt: $grp -> rij overgeslagen"; continue }
        ...
        if (Test-Path $folder) {
            try {
                ...
                Write-Log "NTFS '$ntfs' op $folder voor $id"
            } catch { Write-Log "FOUT NTFS $folder / ${grp}: $($_.Exception.Message)" }
        } else { Write-Log "Map niet gevonden, NTFS overgeslagen: $folder" }
        ...
        if ($s) {
            try {
                ...
                Write-Log "Share '$share' -> $id : $sperm"
            } catch { Write-Log "FOUT share access $share / ${grp}: $($_.Exception.Message)" }
        } else { Write-Log "Share ontbreekt: $share (groep $grp)" }
    }
}
